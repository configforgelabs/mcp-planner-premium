...
const SERVER_INSTRUCTIONS = [
  ...
  "Custom Dataverse columns note: customer-added (non-msdyn_) columns work on-demand by default (no setup) — call list_custom_columns / describe_columns to discover what exists on this tenant BEFORE using customFields on add_tasks / update_tasks / create_plan or includeCustomColumns on get_task / list_plan_tasks / get_plan_summary; never guess a custom column's name or type. (An operator may disable the feature with CUSTOM_COLUMNS_MODE=off.)",
].join("\n");
