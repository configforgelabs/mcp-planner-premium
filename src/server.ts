...
const SERVER_INSTRUCTIONS = [
  ...
  "Custom Dataverse columns note: customer-added (non-msdyn_) columns work on-demand by default (no setup) — call list_custom_columns / describe_columns to discover what exists on this tenant BEFORE using customFields on add_tasks / update_tasks / create_plan or includeCustomColumns on get_task / list_plan_tasks / get_plan_summary; never guess a custom column's name or type. (An operator may disable the feature with CUSTOM_COLUMNS_MODE=off.) This only covers columns that exist as DATAVERSE columns on the Project/Task tables (e.g. added via Power Apps). Custom fields created in the Microsoft Planner APP itself (the Planner web 'Edit field' UI) are Planner-service/UI-only: they are NOT exposed by any API - not this Dataverse server, and NOT Microsoft Graph either (verified against the live Graph schema; the Planner Graph API has no custom-field resource) - so they cannot be read or written programmatically at all, and list_custom_columns will not return them. To automate such data today, encode it in the task subject/label (writable via add_tasks/update_tasks), or add it as a real Dataverse column on the Project/Task tables via Power Apps (which this server then handles).",
].join("\n");
