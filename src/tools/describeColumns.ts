...
export const describeColumns: ToolDef = {
  ...
  description:
    "Returns deep detail for named customer-added Dataverse columns on the plan (project) or task entity: normalized type, create/update validity, computed flag, option-set values+labels (picklist/multiselect), date format, and lookup navigation-property/target-entity/entity-set info. A superset of describe_option_set for the custom-column case (describe_option_set still works for standard fields). Read-only. On-demand by default; returns ok:false only if the operator disabled custom columns (CUSTOM_COLUMNS_MODE=off). Dataverse columns only. Fields created in the Microsoft Planner app itself are UI-only and are NOT exposed by any Dataverse or Microsoft Graph API (verified against the live Graph schema), so they can't be read/written programmatically and won't appear here; encode such data in the task subject/label, or add a real Dataverse column via Power Apps.",
  inputSchema: {
