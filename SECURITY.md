...
| 5.6 Custom Dataverse column metadata privilege | ✅ Done | Discovering/reading/writing custom (non-`msdyn_`) columns (on-demand by default, §11) requires only `prvReadEntity`/`prvReadAttribute` on entity/attribute metadata — a read privilege normally held by any user who can read the record at all, so no elevated grant is needed. If the tenant locks this down and the metadata read 403s, custom-column writes fail closed with an actionable error (§11); standard `msdyn_` fields are entirely unaffected either way. |
...
Custom (customer-added, non-`msdyn_`) Dataverse columns work **on-demand by
default** — nothing runs until a caller references one; `CUSTOM_COLUMNS_MODE=off`
is the opt-out kill switch (see [`README.md`](README.md#custom-dataverse-columns)
for the full feature description). Its security model:

| Control | Status | Implementation / reason |
|---|---|---|
| On-demand by default, fully additive | ✅ Done | The feature only does work when a caller references a custom column (`customFields` / `includeCustomColumns` / a non-`msdyn_` raw key); a request touching no custom column is byte-for-byte unchanged with zero added metadata cost. `CUSTOM_COLUMNS_MODE=off` disables it entirely (opt-out kill switch) ([`src/config.ts`](src/config.ts)). |
...
| Allowlist enforced symmetrically (read + write) | ✅ Done | In `metadata+allowlist` mode, `CUSTOM_COLUMNS_ALLOWLIST` restricts which custom columns are usable on **both** reads and writes (and the raw-batch guard), via the shared `applyAllowlist` ([`src/tools/customColumnsRead.ts`](src/tools/customColumnsRead.ts)) applied to the metadata before the write resolver / guard sees it — a non-allowlisted column is rejected on write, not merely hidden on read. |
