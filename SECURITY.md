...
| 2.6 | Confused-deputy prevention | ⚠️ Partial | `appid`/`azp` pinning (`LANGDOCK_CLIENT_ID`) ensures the token came from our app; network ingress restriction is an operator control (§5.3). Full prevention requires OBO (§9). |
