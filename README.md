...
> **Dataverse only — and Planner-app fields aren't reachable by any API.** This
> section covers columns that exist as **Dataverse columns** on the
> `msdyn_project` / `msdyn_projecttask` tables (e.g. added via Power Apps).
> Custom fields created in the **Microsoft Planner app itself** (the Planner web
> "Edit field" UI) are Planner-service/UI-only: they are **not** Dataverse
> columns, and — verified against the live Microsoft Graph schema — they are
> **not exposed by Microsoft Graph either** (the Planner Graph API has no
> custom-field resource). So they **cannot be read or written programmatically
> at all**, and `list_custom_columns` will not return them. To automate such
> data today, encode it in the task **subject/label** (writable via
> `add_tasks`/`update_tasks`), or add it as a real **Dataverse column** via
> Power Apps (which this server then handles).
